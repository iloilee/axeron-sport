            ...
            const modalContent = `
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-bold">Chi Tiết Đơn Hàng ${o.order_code}</h2>
...
function exportToCSV(orders) {
    ...
    orders.forEach(order => {
        ...
        csv += `"${order.order_code}","${order.recipient_name}","${order.recipient_phone}","${address}",${order.total_amount},"${order.payment_status_text}","${order.order_status_text}","${order.created_at}","${products}"\n`;
    });
...
function updateOrderStatusFromDropdown(orderId, newStatus) {
    ...
    showConfirm(`Cập nhật đơn hàng này sang trạng thái "${statusLabels[newStatus]}"?`, async () => {
        const formData = new FormData();
...
function updatePaymentStatus(orderId, newStatus) {
    ...
    showConfirm(`Cập nhật trạng thái thanh toán đơn hàng này sang "${statusLabels[newStatus]}"?`, async () => {
        const formData = new FormData();
