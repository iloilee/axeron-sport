...
if (empty($_SESSION['reg_reset_token']) || empty($_SESSION['reg_email'])) {
    setFlash('error', 'Vui lòng nhập email để nhận mã xác thực');
    redirect(BASE_URL . '/auth/login.php');
}

$resetToken = $_SESSION['reg_reset_token'];
$resetEmail = $_SESSION['reg_email'];
        ...
        <div class="bg-white rounded-2xl shadow-2xl p-8 w-full border border-[#e5e2e1]">
            ...
            <div class="mt-6 text-center">
                <p class="text-[#5b403f] text-sm">
                    Không nhận được mã?
                    <a href="<?= BASE_URL ?>/auth/login.php" class="text-[#BE1E2D] hover:text-[#98001b] transition-colors font-semibold ml-1">
                        Gửi lại
                    </a>
                </p>
            </div>
