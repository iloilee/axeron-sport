...
async function exportToPDF() {
    try {
        ...
        doc.setFontSize(10);
        doc.text('Tong doanh thu: ' + cleanTextForPdf(revenue), 10, 35);
        doc.text('Tong don hang: ' + cleanTextForPdf(orders), 10, 42);
        ...
        doc.autoTable({
            ...
            didParseCell: function(data) {
                if (data.cell.text && Array.isArray(data.cell.text)) {
                    data.cell.text = data.cell.text.map(t => cleanTextForPdf(t));
                } else if (typeof data.cell.text === 'string') {
                    data.cell.text = cleanTextForPdf(data.cell.text);
                }
            }
        });
...
/**
 * Helper: Clean Text for PDF (Remove Vietnamese Tones & Emojis)
 */
function cleanTextForPdf(str) {
    if (!str || typeof str !== 'string') return str;
    
    // Remove emojis and special symbols
    str = str.replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F700}-\u{1F77F}\u{1F780}-\u{1F7FF}\u{1F800}-\u{1F8FF}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{2B50}]/gu, '');
